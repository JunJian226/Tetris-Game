export {rngBlocks, createSvgElement, noOfLargerNumber, isFinal, highestPoint, rotatePoint}

import {LazySequence, possibleTetromino, blokProp, Viewport} from "./types"
...
const isFinal = (blocks: ReadonlyArray<blokProp>): boolean =>
    blocks.map( (ele) => ele.label).reduce( (acc,ele)=> acc || ele.includes("final") ,false)
  
const highestPoint =(blocks: ReadonlyArray<blokProp>): number =>
    blocks.reduce((acc: number,ele)=> (+ele.y<acc)?+ele.y:acc,Viewport.CANVAS_HEIGHT)

/**     Uses 2D Rotation Transformation Formula */
